...
public class Reconciliation {
	...
	public static double[] varianceArray(int size) {
		double[] varianceVector = new double[size];
		for (int i = 0; i < size; i++) {
			varianceVector[i] = 0.000001;
		}
		return varianceVector;
	}

	public static double[] incidenceMatrix(int numFlux) {
		double[] matrixA = new double[numFlux];
		matrixA[0] = 1;
		for (int i = 1; i < numFlux; i++) {
			matrixA[i] = -1;
		}
		return matrixA;
	}
